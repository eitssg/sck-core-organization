...
def resolve_parent_id(parent_id: str, organizations_client: Any) -> str:
    ...
    if parent_id.lower() == "root":
        try:
            ...
            log.debug(
                "Resolved root parent ID",
                details={"original": parent_id, "resolved": resolved_id},
            )
            return resolved_id
    ...
    try:
        ...
        log.info(
            ...
            details={
                ...
                "ou_ids": [ou.get("Id") for ou in ou_list],
            },
        )
        ...
        send_success_response(
            ...
            physical_resource_id=f"Listed-{resolved_parent_id}",
        )
...
def create_organizational_unit(event: Dict[str, Any], context: Any) -> None:
    ...
    try:
        ...
        if (
            ...
            for child_account_id in children:
                try:
                    if not validate_account_id(str(child_account_id)):
                        log.warning(
                            "Invalid child account ID format",
                            details={"account_id": child_account_id, "ou_id": ou_id},
                        )
                        continue
                    ...
                    if not parent_response.get("Parents"):
                        log.warning(
                            ...
                            details={"account_id": child_account_id},
                        )
                        continue
            ...
            log.info(
                "Completed child account movements",
                details={"ou_id": ou_id, "child_count": len(children)},
            )
        ...
        log.error(
            ...
            details={
                "error": str(e),
                "error_type": type(e).__name__,
                "ou_name": event.get("ResourceProperties", {}).get("Name"),
            },
        )
        send_failure_response(event, context, error_msg)
...
def update_organizational_unit(event: Dict[str, Any], context: Any) -> None:
    ...
    log.info(
        "Starting organizational unit update",
        details={"ou_id": ou_id, "logical_resource_id": event.get("LogicalResourceId")},
    )
    ...
    try:
        ...
        log.info(
            ...
            details={"ou_id": ou_id, "ou_arn": ou_arn, "new_name": ou_name},
        )
        ...
        log.error(
            "OU update validation failed",
            details={"ou_id": ou_id, "error": str(validation_error)},
        )
        ...
        log.error(
            "Organizational unit update failed",
            details={"ou_id": ou_id, "error": str(e), "error_type": type(e).__name__},
        )
        send_failure_response(event, context, error_msg, ou_id)
...
def move_all_children_to_root(ou_id: str, organizations_client: Any) -> None:
    ...
    try:
        ...
        log.info(
            "Moving child accounts to root",
            details={"ou_id": ou_id, "root_id": root_id, "child_count": len(children)},
        )
        ...
        log.info(
            "Successfully moved all children to root",
            details={"ou_id": ou_id, "child_count": len(children)},
        )
...
def delete_organizational_unit(event: Dict[str, Any], context: Any) -> None:
    ...
    log.info(
        "Starting organizational unit deletion",
        details={"ou_id": ou_id, "logical_resource_id": event.get("LogicalResourceId")},
    )
    ...
    try:
        ...
        try:
            ...
            log.error(
                "Failed to move children to root",
                details={"ou_id": ou_id, "error": str(move_error)},
            )
        ...
        log.error(
            "Organizational unit deletion failed",
            details={"ou_id": ou_id, "error": str(e), "error_type": type(e).__name__},
        )
        send_failure_response(event, context, error_msg, ou_id)
...
def move_account_between_ous(event: Dict[str, Any], context: Any) -> None:
    ...
    """
    log.info(
        "Starting account move operation",
    ...
    try:
        ...
        if resolved_source_id == resolved_destination_id:
            log.info(
                "Account already in destination parent",
                details={
                    "account_id": account_id,
                    "parent_id": resolved_destination_id,
                },
            )
            ...
            send_success_response(
                ...
                context=context,
                response_data=response_data,
                physical_resource_id=f"AccountMove-{account_id}",
            )
            return
